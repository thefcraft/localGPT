...
function LocalGPT(prompt){
    ...
    fetch('/api/generate', {
        ...
        body: JSON.stringify({'prompt': prompt,
                            'chat_id': window.location.pathname.split('/c/')[1]})
    })
        .then(response => {
                ...
                start(controller) {
                    function pump() {
                        ...
                        reader.read().then(({ done, value }) => {
                            ...
                            // promptResponse += characters.replace('```', '');
                            ...
                            last_code_block = document.getElementById('code-block-last');
                            if(last_code_block != null) {
                                if(characters.match(/```/g)){
                                    last_code_block.innerHTML += characters.split('```')[0];
                                    outputDiv.innerHTML += characters.split('```')[1];
                                    document.getElementById('code-block-last').removeAttribute('id');
                                }else{
                                    last_code_block.innerHTML += characters;
                                    delete last_code_block.dataset.highlighted;
                                    hljs.highlightElement(document.getElementById('code-block-last'));
                                }
                            }else{
